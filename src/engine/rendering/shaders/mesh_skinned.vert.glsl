...
// Bone palette; bound with a dynamic offset per draw. Size must match
// kMaxModelBones in model_asset.hpp.
...
void main() {
  ...
  vec4 world = draw.model * (skinMat * vec4(inPosition, 1.0));
  vWorldPos = world.xyz;
  vWorldNormal = normalize(mat3(draw.model) * (mat3(skinMat) * inNormal));
  vUV = inUV;
